...
**Note:** (?) Levels are numbered automatically from the level with the smallest time unit, which is always shown at the bottom of the header, to the level with the largest one, which is shown on the top. For example, the default levels – the month, quarter, and year – are assigned the indexes 0, 1, 2.
