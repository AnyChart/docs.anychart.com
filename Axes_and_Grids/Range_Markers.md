...
To add range marker you need to use {api:anychart.charts.Cartesian#rangeMarker}**.rangeMarker()**{api} method. By default this method creates a marker on a primary y axis. You can bind the marker to another axis using {api:anychart.core.axisMarkers.Range#axis}**.axis()**{api} method. The basic range marker's properties can be set using {api:anychart.core.axisMarkers.Range#layout#from}**.from()**{api}, {api:anychart.core.axisMarkers.Range#layout#to}**to()**{api} and {api:anychart.core.axisMarkers.Range#layout#fill}**.fill()**{api} methods.
...
Along with binding marker to an axis, it is possible to use a scale for marker positioning. The scale for the marker can be defined using {api:anychart.core.axisMarkers.Range#layout#scale}**.scale()**{api} method. As far as scales don't have any layout you need to define layout for the marker using {api:anychart.core.axisMarkers.Range#layout#layout}**.layout()**{api} method.
