...
One of the ways to create items for a standalone legend is calling the {api:anychart.standalones.Legend#itemsSource}itemsSource(){api} method with an array of source items as a parameter. Normally, this method takes an array of charts, which allows [binding one legend to multiple charts](../Common_Settings/Legend/Standalone_Legend#item_=_series_/_point). However, in AnyStock, it requires specifying an array of plots – so the legend is linked to multiple plots, legend items representing the series of these plots.
