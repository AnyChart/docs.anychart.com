...
To build a Vertical Stacked Spline Area Chart, create a multiple-series [Vertical Spline Area Chart](../../vertical/spline-area-chart) and set the {api:anychart.scales.Linear#stackMode}stackMode(){api} method into **value**:
...
The Vertical Stacked Spline Area series' settings are mostly the same as other series' ones. The majority of information about adjusting series in AnyChart is given in the [General Settings article](../../general-settings).