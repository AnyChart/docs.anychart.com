...
If a tree does not fit the height of the chart, children of some or all items are hidden. After each such item, the number of its children is shown, followed by a text element – postfix. You can configure the text: pass a custom string to the {api:anychart.charts.Wordtree#postfix}postfix(){api} method. By default, the postfix is `"more"`.
