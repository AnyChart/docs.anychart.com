...
Points padding is a special setting in Mekko charts, it is controlled with {api:anychart.charts.Mekko#pointsPadding}pointsPadding(){api} method and sets the distance between elements (tiles), it serves only aesthetic purpose and it is set different default values in [Mekko](Mekko_Chart), [BarMekko](BarMekko_Chart) and [Mosaic](Mosaic_Chart) constuctors.
