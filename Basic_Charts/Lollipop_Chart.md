...
The tooltip has a title and a text body. To set the title, use {api:anychart.core.ui.Tooltip#titleFormat}titleFormat(){api} on the tooltip of the chart; to set the text, use {api:anychart.core.ui.Tooltip#format}format(){api} on the tooltip of the series. Text formatters: `{%x}` is the category, `{%value}` is the value. When several series share a category, one tooltip lists them all:
