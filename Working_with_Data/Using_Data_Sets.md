...
```
   ...
   | 2014/6/30 |    28    |      25       |     27      |
```
In many cases you are forced to create three sets to create a line chart with the three series:
```
...
###Creating Data Set
Data Set works with a list of rows.  
Each row can be represented by one of the following types:  
####Number or String  
In this case each row represents a point value.  
Other point properties such as `x`, `index`, `name`, etc will be autogenerated.
```
var data = [
// column 0
    28,   //row 0
...
```
####Array
If you want to create more than one series or a chart with similar data it is recommended to use `Array` as a row. Zero column values of the each row will represent as a series X (argument), each of the following columns value will  represent a series value.  
