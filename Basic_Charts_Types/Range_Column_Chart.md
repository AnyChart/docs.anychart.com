...
<tr><td></td><td><a href="http://www.anychart.com/chartopedia/chart-types/range-column-chart/" target="_blank">Chartopedia: Range Column Chart</a></td></tr>
...
Since range column charts plot two Y-values per data point, you need to specify two values for each category by using the <strong>"low"</strong>strong> and <strong>"high"</strong> parameters. That is how it looks like in object notation:
...
If you set {api:anychart.charts.Cartesian#barsPadding}barsPadding(){api} to -1, you can create an imitation of the [Stacked Column](Stacked/Value/Column_Chart) chart, which is not bound to zero. See the sample below (there is a multi-series Range Column chart with the padding between columns and between column groups set to -1 and 2):
...
To configure a label on a Range Column chart, you need to know the following peculiarities regarding formatting and positioning lables.... 
