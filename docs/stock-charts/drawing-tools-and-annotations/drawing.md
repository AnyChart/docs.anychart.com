...
- {api:anychart.enums.EventType}annotationDrawingFinish{api} - this event can be used to track newly created (or changed) annotations and sending the list of annotations (or the new one) to a server. See information about annotation events in the [Handling events section](#handling-events).
