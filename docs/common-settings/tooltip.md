...
Formatting title and separator is described the [title formatting section](#title-formatting). 
...
This feature allows you to usa any possible HTML settings: you can adjust background color, font style and size, the separator, and so on - use [inline styles](#inline-styles) or [CSS classes](#css-classes). Tooltips are rendered as separate HTML elements added to DOM. 
...
In the context, all methods get links to the four HTML elements of the tooltip (see the [CSS Classes](#css-classes) section). For example, this is how the context of {api:anychart.core.ui.Tooltip#onDomReady}onDomReady(){api} looks like:
