...
The way of managing chart's points is pretty much the same as the one in [single point](#single-point) section. Use {api:anychart.core.Point#hovered}hovered(){api} and {api:anychart.core.Point#selected}selected(){api} methods to adjust current state of any point. The {api:anychart.core.Point#set}set(){api} method allows you to change or create any property (field) of a point.
