...
**Note 1:** A Word Tree can have only one [root word](#root-word).
...
**Note:** If necessary, you can set the [root word](#root-word) manually.
...
Setting data as a text requires setting the root [root word](#root-word) manually by using the {api:anychart.charts.Wordtree#word}word(){api} method. Then AnyChart automatically builds a tree with the specified root. In the following sample, it is the word `"He"`:
