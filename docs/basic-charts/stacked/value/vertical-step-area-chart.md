...
To build a Vertical Stacked Step Area Chart, create a multiple-series [Vertical Step Area Chart](../../vertical/step-area-chart) and set the {api:anychart.scales.Linear#stackMode}stackMode(){api} method into **value**:
...
The Vertical Stacked Step Area series' settings are mostly the same as other series'. The majority of information about adjusting series in AnyChart is given in the [General Settings article](../../general-settings).