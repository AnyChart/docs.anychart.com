...
There are two ways to set data to a Surface chart: pass an array to the {api:anychart#surface}anychart.surface(){api} chart constructor, like in the [Quick Start](#quick-start) sample, or to the {api:anychart.charts.Surface#data}data(){api} method.
...
When the [color scale](#color-scale) is used, you can add a color range to make chart more readable. It is created with the {api:anychart.charts.Surface#colorRange}colorRange(){api} method:
