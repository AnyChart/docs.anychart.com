...
The first file is the AnyChart UI stylesheet. The third file holds the AnyChart icon font. The icon font draws the small icons in the [context menu](Keyboard_and_Clipboard#context_menu) - the menu that opens when you right-click. Without the font, every menu row shows an empty space instead of its icon.
